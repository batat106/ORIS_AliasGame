...
public class GameManager
{
    ...
    public void RemovePlayer(string playerId)
    {
        ...
        {
            // Если вообще не осталось игроков, игра заканчивается
            _gameState.CurrentPlayer = null;
            _gameState.CurrentWord = null;
            _gameState.TimeLeft = 0;
        }
        else if (_gameState.Players.Count == 1)
        {
            // Если остался один игрок, он выигрывает, игра останавливается
            _gameState.CurrentPlayer = _gameState.Players.First();
            _gameState.CurrentWord = null; 
            _gameState.TimeLeft = 0;
        }
        else
        {
            // Если удалённый игрок был текущим, переключаемся на следующего
        ...
        // Запускаем таймер только если осталось 2+ игроков
        if (_gameState.Players.Count > 1)
        {
            _timer.Start();
        }

        // Обновляем состояние игры (например, обновление UI)
        OnGameStateUpdated?.Invoke(_gameState);
    }
